...
const getDefaultCategories = (t: any): PackingCategory[] => [
  {
    id: 'docs',
...
export default function PackingListModal({ visible, tripId, onClose }: { visible: boolean; tripId: string; onClose: () => void; }) {
  const dispatch = useDispatch();
  const { t } = useTranslation();
  const packingList = useSelector((state: RootState) =>
    state.packing.packingLists.find((p) => p.tripId === tripId)
  );
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput
              style={styles.modalInput}
              placeholder={t.enterCategoryIcon}
