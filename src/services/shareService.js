...
export class ShareService {
     ...
     */
    static async generateInviteLink(tripId) {
        try {
     ...
     */
    static async shareWithUsers(tripId, userIds) {
        try {
            const tripRef = doc(db, 'trips', tripId);
            ...
            const newSharedWith = mergedArray.filter((item, index) => mergedArray.indexOf(item) === index);
            // Update trip
            await updateDoc(tripRef, {
     ...
     */
    static async updateSharingSettings(tripId, options) {
        try {
            const tripRef = doc(db, 'trips', tripId);
            await updateDoc(tripRef, {
     ...
     * Make a trip public
     */
    static async makePublic(tripId) {
        try {
            const tripRef = doc(db, 'trips', tripId);
            await updateDoc(tripRef, {
                visibility: 'public',
