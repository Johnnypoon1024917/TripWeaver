...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Platform, Animated, PanResponder, Alert, TextInput, RefreshControl, Keyboard, } from 'react-native';
...
import { addDay, addDestination, deleteDestination, reorderDestinations, updateDay, deleteDay } from '../store/slices/itinerarySlice';
...
export default function ItineraryScreen({ route }) {
    ...
    const [segmentTravelModes, setSegmentTravelModes] = useState(new Map());
    ...
    const destinations = useSelector((state) => state.itinerary.days.flatMap(day => day.destinations));
    // Services
    ...
    const handleEditDay = (dayId, currentTitle) => {
    ...
    const renderItem = ({ item, drag, isActive }) => {
        ...
        <Animated.View>
          <TouchableOpacity style={styles.destItem} onPress={() => navigation.navigate('MainTabs', { screen: 'Map' })} onLongPress={drag} disabled={isActive}>
            ...
            <View style={styles.destInfo}>
              ...
              <View style={styles.destMeta}>
                ...
                {item.cost && (<Text style={styles.destCost}>💰 ${item.cost}</Text>)}
              </View>
              <Text style={styles.destAddress} numberOfLines={2}>
                📍 {item.address}
              </Text>
              {item.notes && (<Text style={styles.destNotes} numberOfLines={2}>
                  📝 {item.notes}
                </Text>)}
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.mainContent}>
        ...
        <View style={[styles.leftPanel, { width: leftPanelWidth }]}>
          <ScrollView ref={scrollViewRef} style={styles.timeline} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh}/>}>
            {/* Mobile Day Tabs */}
            {windowWidth < 768 && (<ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.dayTabs}>
                {days.map((day) => (<TouchableOpacity key={`day${day.dayNumber}`} style={[
                    styles.dayTab,
            ...
            {/* Desktop Day Sidebar */}
            {windowWidth >= 768 && (<View style={styles.daySidebar}>
                ...
                <TouchableOpacity style={styles.addDayButton} onPress={() => {
                const newDay = {
            ...
            {/* Day Content */}
            {currentDayData && (<View style={styles.dayContent}>
                {/* Day Header */}
                <View style={styles.dayHeader}>
                  <View style={styles.dayHeaderLeft}>
                    {isEditing && editingDayId === `day${currentDayData.dayNumber}` ? (<TextInput style={styles.dayTitleInput} value={editedDayTitle} onChangeText={setEditedDayTitle} onSubmitEditing={saveEditedDay} autoFocus/>) : (<Text style={styles.dayTitle}>{`Day ${currentDayData.dayNumber}`}</Text>)}
                ...
                day: 'numeric'
                  ...
                  <View style={styles.dayActions}>
                    <TouchableOpacity style={styles.dayActionButton} onPress={() => handleEditDay(`day${currentDayData.dayNumber}`, `Day ${currentDayData.dayNumber}`)}>
                      <Ionicons name="pencil" size={16} color={colors.textSecondary}/>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.dayActionButton} onPress={() => handleDeleteDay(`day${currentDayData.dayNumber}`)}>
                ...
                {/* Weather Widget */}
                <CollapsibleSection title={'Weather'} expanded={expandedSections['weather']} onToggle={() => toggleSection('weather')}>
                  <WeatherWidget location={selectedTrip.destination} date={currentDayData.date}/>
                </CollapsibleSection>
                
                {/* Destinations List */}
                <CollapsibleSection title={`${'Destinations'} (${currentDayData.destinations.length})`} expanded={expandedSections['destinations']} onToggle={() => toggleSection('destinations')}>
                  {currentDayData.destinations.length > 0 ? (<>
                      <DraggableFlatList ref={dragRef} data={currentDayData.destinations} renderItem={renderItem} keyExtractor={(item) => item.id} onDragEnd={handleDragEnd} activationDistance={20}/>
                      ...
                      <TouchableOpacity testID="add-destination-button" style={styles.emptyTimelineButton} onPress={() => setShowAddDestModal(true)}>
                ...
                {/* Add Buttons */}
                {currentDayData.destinations.length > 0 && (<View style={styles.addButtonsRow}>
                    <TouchableOpacity testID="add-destination-button" style={styles.addButton} onPress={() => setShowAddDestModal(true)}>
        ...
        {windowWidth >= 768 && (<View {...rightDividerPan.panHandlers} style={styles.resizeDivider}>
            <View style={styles.resizeHandle}/>
          </View>)}
        
        {/* RIGHT PANEL - Map */}
        {windowWidth >= 768 && (<View style={styles.rightPanel}>
            {Platform.OS !== 'web' ? (<MapView ref={(ref) => mapRef.current = ref} style={styles.map} region={getMapRegion()} showsUserLocation showsMyLocationButton={false}>
                {currentDayData?.destinations.map((dest, index) => (<Marker key={dest.id} coordinate={{
                ...
                {currentDayData && currentDayData.destinations.length > 1 && (<Polyline coordinates={currentDayData.destinations.map(d => ({
                        latitude: d.latitude,
      ...
      {/* Modals */}
      <AddDestinationModal visible={showAddDestModal} onClose={() => setShowAddDestModal(false)} onAddDestination={(destination) => {
            if (currentDayData) {
      ...
      <PackingListModal visible={showPackingList} onClose={() => setShowPackingList(false)} tripId={selectedTrip.id}/>
      
      {/* Floating Add Button (Mobile) */}
      {windowWidth < 768 && (<TouchableOpacity style={styles.floatingAddButton} onPress={() => setShowAddDestModal(true)}>
