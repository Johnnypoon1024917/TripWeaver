...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, ScrollView, Dimensions, Platform, Animated, Alert, } from 'react-native';
...
const TypedMapboxGL = MapboxGL;
export default function MapboxMapScreen({ navigation }) {
    ...
    const fitToDestinations = () => {
        if (allDestinations.length > 0 && cameraRef.current) {
            ...
            cameraRef.current.fitBounds([Math.min(...coordinates.map(c => c[0])), Math.min(...coordinates.map(c => c[1]))], [Math.max(...coordinates.map(c => c[0])), Math.max(...coordinates.map(c => c[1]))], [50, 50, 50, 300], 1000);
        }
    };
    ...
    const downloadOfflineMap = async () => {
        ...
        if (Platform.OS === 'web') {
            Alert.alert('Not Supported', 'Offline maps are not supported on web platform. Please use the mobile app for this feature.');
            return;
        }
        ...
        try {
            ...
            const pack = await MapboxGL.offlineManager.createPack({
                ...
                console.log('Download progress:', progress.percentage || progress);
            });
            setOfflinePack(pack);
    ...
    return (<View style={styles.container}>
      {Platform.OS === 'web' ? (
        ...
        <MapboxGL.MapView ref={mapRef} style={styles.map} styleURL={MapboxGL.StyleURL.Street} zoomEnabled={true} scrollEnabled={true} pitchEnabled={true} rotateEnabled={true}>
          <MapboxGL.Camera ref={cameraRef} defaultSettings={{
                centerCoordinate: [region.longitude, region.latitude],
                zoomLevel: 12,
            }}/>
          
          {/* Current day destinations */}
          {currentDayDestinations.map((dest, index) => (<MapboxGL.PointAnnotation key={dest.id} id={`destination-${dest.id}`} coordinate={[dest.longitude, dest.latitude]}>
          ...
          {selectedPlace && (<MapboxGL.PointAnnotation id={`selected-place-${selectedPlace.id}`} coordinate={[selectedPlace.longitude, selectedPlace.latitude]}>
          ...
          {/* Route line */}
          {currentDayDestinations.length > 1 && (<MapboxGL.ShapeSource id="routeSource" shape={{
                    type: 'Feature',
                    properties: {},
                    geometry: {
                        type: 'LineString',
      ...
      <View style={styles.searchContainer}>
        ...
        {showSearch && searchResults.length > 0 && (<Animated.View style={[
            ...
            <ScrollView>
              {searchResults.map((place) => (<TouchableOpacity key={place.id} style={styles.resultItem} onPress={() => selectPlace(place)}>
      ...
      <View style={styles.bottomPanel}>
        <View style={styles.controlsRow}>
          <TouchableOpacity style={styles.controlButton} onPress={optimizeDayRoute}>
            <Text style={styles.controlButtonText}>🧭 Optimize</Text>
          </TouchableOpacity>
          
          <TouchableOpacity style={styles.controlButton} onPress={downloadOfflineMap} disabled={isDownloading}>
      ...
      {currentDayDestinations.length > 0 && (<View style={styles.destinationsPanel}>
          <Text style={styles.destinationsTitle}>Drag to Reorder</Text>
          <DraggableFlatList data={currentDayDestinations} renderItem={renderDraggableItem} keyExtractor={(item) => item.id} onDragEnd={({ data }) => {
