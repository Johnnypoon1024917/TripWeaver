...
export default function ExploreScreen({ navigation, route }: any) {
  ...
  const handleAddToTrip = (place: Place, dayNumber?: number) => {
    if (!selectedTrip) {
  ...
  const mapPlaceTypeToCategory = (types?: string[]): any => {
    ...
    if (types.includes('bar') || types.includes('night_club')) return 'activity';
