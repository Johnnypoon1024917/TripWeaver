...
type TripDetailNavigationProp = CompositeNavigationProp<
  StackNavigationProp<RootStackParamList, 'TripDetail'>,
...
export default function TripDetailScreen() {
  ...
  const itinerary = useSelector((state: RootState) =>
    state.itinerary.days.filter((day) => day.tripId === tripId)
  );
      ...
      <View style={styles.content}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.mapContainer}>
              <MapView
                ...
                rotateEnabled={false}
              >
                {Platform.OS !== 'web' && allDestinations.map((dest, index) => (
                  <Marker
                    key={dest.id}
                    coordinate={{
        ...
        <View style={styles.actions}>
          ...
          <TouchableOpacity onPress={handleViewMap}>
            <LinearGradient
              colors={[colors.secondary, '#95E1D3']}
              style={styles.actionButton}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
            >
