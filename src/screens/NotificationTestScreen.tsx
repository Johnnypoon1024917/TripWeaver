...
const NotificationTestScreen: React.FC = () => {
  ...
  const sendTestNotification = async () => {
    try {
      await NotificationUtils.sendGeneralNotification(
        'Test Notification',
        'This is a test notification from TripWeaver!',
        { test: true 
      );
  ...
  const sendTripReminder = async () => {
    try {
      await NotificationUtils.sendTripReminder(
        'test-trip-id',
        'Test Trip to Paris',
        3
      );
  ...
  const sendBudgetAlert = async () => {
    try {
      await NotificationUtils.sendBudgetAlert(
        'test-trip-id',
        'Accommodation',
        850,
        1000
  ...
  const sendCollaborationNotification = async () => {
    try {
      await NotificationUtils.sendCollaborationNotification(
        'test-trip-id',
        'Added a new destination to the itinerary',
        'John Doe'
      );
  ...
  const sendWeatherAlert = async () => {
    try {
      await NotificationUtils.sendWeatherAlert(
        'test-trip-id',
        'Paris, France',
        'Thunderstorms',
        'high'
