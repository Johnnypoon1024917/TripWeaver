...
export default function EnhancedBudgetScreen({ navigation, route ) {
    const dispatch = useDispatch();
    const budgets = useSelector((state) => state.budget.budgets);
    const expenses = useSelector((state) => state.budget.expenses);
    const collaborators = useSelector((state) => state.trips.selectedTrip?.collaborators || []);
    ...
    const renderSplitDetails = () => {
        ...
        return (<View style={styles.splitDetailsContainer>
        <Text style={styles.sectionTitle>Split Details</Text>
        {collaborators.map(userId => (<View key={userId style={styles.splitDetailRow>
            <Text style={styles.splitUserLabel>User {userId.substring(0, 5)...</Text>
            {newExpense.splitType === 'percentage' && (<TextInput style={styles.splitInput placeholder="Percentage" value={splitDetails[userId]?.percentage onChangeText={text => {
                        setSplitDetails({
                            ...splitDetails,
                            [userId]: { ...splitDetails[userId], percentage: text ,
                        );
                     keyboardType="numeric"/>)
            {newExpense.splitType === 'exact' && (<TextInput style={styles.splitInput placeholder="Amount" value={splitDetails[userId]?.amount onChangeText={text => {
                        setSplitDetails({
                            ...splitDetails,
                            [userId]: { ...splitDetails[userId], amount: text ,
                        );
                     keyboardType="numeric"/>)
            {newExpense.splitType === 'shares' && (<TextInput style={styles.splitInput placeholder="Shares" value={splitDetails[userId]?.shares onChangeText={text => {
                        setSplitDetails({
                            ...splitDetails,
    ...
    return (<View style={styles.container>
      ...
      <ScrollView style={styles.content>
        <View style={styles.summaryCard>
          ...
          <View style={styles.chartsContainer>
            <View style={styles.chartWrapper>
              ...
              {budgets.length > 0 ? (<PieChart style={styles.chart data={budgets.map((budget, index) => ({
                key: budget.id,
                value: budget.amount,
                svg: {
                    fill: [
                ...
                ,
                arc: { cornerRadius: 2 ,
            )) innerRadius={40 outerRadius={80/>) : (<View style={styles.emptyChart>
            ...
            <View style={styles.chartWrapper>
              ...
              {budgets.length > 0 ? (<BarChart style={styles.chart data={budgets.map(budget => ({
                value: budget.spent,
                label: budget.category.substring(0, 3),
            )) svg={{ fill: colors.primary  contentInset={{ top: 20, bottom: 20, left: 20, right: 20 />) : (<View style={styles.emptyChart>
                  <Text style={styles.emptyChartText>No data</Text>
                </View>)
            </View>
          </View>
        ...
        <View style={styles.section>
          <View style={styles.sectionHeader>
            ...
            <TouchableOpacity style={styles.addButton onPress={() => setShowAddBudgetModal(true)>
        ...
        <View style={styles.section>
          <View style={styles.sectionHeader>
            ...
            <TouchableOpacity style={styles.addButton onPress={() => setShowAddExpenseModal(true)>
              <Text style={styles.addButtonText>+ Add</Text>
            </TouchableOpacity>
          </View>
          ...
          {expenses.length === 0 ? (<Text style={styles.emptyText>No expenses recorded</Text>) : (expenses.slice(0, 10).map((expense) => (<View key={expense.id style={styles.expenseCard>
                <View style={styles.expenseInfo>
                  ...
                  {expense.splitType !== 'even' && (<Text style={styles.splitTypeText>
      ...
      {/* Add Expense Modal */
      <Modal visible={showAddExpenseModal animationType="slide" presentationStyle="pageSheet">
        ...
        >
          ...
          <ScrollView style={styles.modalContent>
            <View style={styles.inputGroup>
              ...
              <TextInput style={styles.input placeholder="What was this expense for?" value={newExpense.description onChangeText={text => setNewExpense({ ...newExpense, description: text )/>
            </View>
            
            <View style={styles.inputGroup>
              <Text style={styles.label>Amount *</Text>
              <TextInput style={styles.input placeholder="0.00" value={newExpense.amount onChangeText={text => setNewExpense({ ...newExpense, amount: text ) keyboardType="numeric"/>
            </View>
            
            <View style={styles.inputGroup>
              <Text style={styles.label>Category *</Text>
              <Picker selectedValue={newExpense.budgetId style={styles.picker onValueChange={(value) => setNewExpense({ ...newExpense, budgetId: value )>
                {budgets.map(budget => (<Picker.Item key={budget.id label={`${budget.category ($${budget.spent.toFixed(2)/${budget.amount.toFixed(2))` value={budget.id/>))
              </Picker>
            </View>
            
            <View style={styles.inputGroup>
              <Text style={styles.label>Paid By</Text>
              <Picker selectedValue={newExpense.paidBy style={styles.picker onValueChange={(value) => setNewExpense({ ...newExpense, paidBy: value )>
                <Picker.Item label="You" value={currentUser?.id || ''/>
                {collaborators.filter(id => id !== currentUser?.id).map(userId => (<Picker.Item key={userId label={`User ${userId.substring(0, 5)...` value={userId/>))
              </Picker>
            </View>
            
            <View style={styles.inputGroup>
              <Text style={styles.label>Split Type</Text>
              <Picker selectedValue={newExpense.splitType style={styles.picker onValueChange={(value) => setNewExpense({ ...newExpense, splitType: value )>
      ...
      {/* Add Budget Modal */
      <Modal visible={showAddBudgetModal animationType="slide" presentationStyle="pageSheet">
        <View style={styles.modalContainer>
          <View style={styles.modalHeader>
        ...
        >
          ...
          <ScrollView style={styles.modalContent>
            <View style={styles.inputGroup>
              <Text style={styles.label>Category *</Text>
              <Picker selectedValue={newBudget.category style={styles.picker onValueChange={(value) => setNewBudget({ ...newBudget, category: value )>
            ...
            <View style={styles.inputGroup>
              <Text style={styles.label>Amount *</Text>
              <TextInput style={styles.input placeholder="0.00" value={newBudget.amount onChangeText={text => setNewBudget({ ...newBudget, amount: text ) keyboardType="numeric"/>
            </View>
            
            <View style={styles.inputGroup>
              <Text style={styles.label>Currency</Text>
              <TextInput style={styles.input placeholder="USD" value={newBudget.currency onChangeText={text => setNewBudget({ ...newBudget, currency: text )/>
            </View>
